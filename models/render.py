...
def sample_pdf(bins, weights, N_importance, det=False, eps=1e-5):
    ...
    pdf = weights / weights_sum  # (N_rays, N_samples_)
    cdf = torch.min(torch.ones_like(pdf), torch.cumsum(pdf, dim=-1))   # (N_rays, N_samples)
...
def render_rays(models,
    ...
    def inference(model, embedding_xyz, xyz_, dir_, dir_embedded, z_vals, weights_only=False):
        ...
        out = torch.cat(out_chunks, 0)   # (N_rays*N_samples, 3 or 4)
        if weights_only:
            sigmas = out.view(N_rays, N_samples_)  # (N_rays, N_samples, 3)
        else:
            rgbsigma = out.view(N_rays, N_samples_, 4)  # (N_rays, N_samples, 4)
